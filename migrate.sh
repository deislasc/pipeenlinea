...
if [ "$row_count" -gt 0 ]; then
    echo ""
    echo -e "${YELLOW}⚠️  ADVERTENCIA: La tabla solicitudes ya tiene datos${NC}"
    echo "Si continúas, se insertarán nuevos registros (puede haber duplicados)"
    echo ""
    read -p "¿Deseas continuar de todas formas? (escribe 'SI' para continuar): " confirm
    if [ "$confirm" != "SI" ]; then
...
case $option in
    1)
        ...
        echo ""
        echo "Si todo se ve bien, ejecuta este script nuevamente y elige la opción 2"
        echo "para hacer la migración real."
        ;;
...
EOF
            ...
            echo ""
            echo "Ahora puedes ejecutar una migración limpia con las opciones 1 o 2"
        else
